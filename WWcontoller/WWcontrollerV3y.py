...
def autodetect_rs485_port(
    candidate_ports: List[str],
    slave_id: int,
    baudrate: int,
) -> Tuple[Optional[str], Optional[ModbusWrapper]]:
    """
    Try each candidate port. A port is considered valid if:
      - connect() succeeds AND
      - a small Modbus read returns data (not None)
    Returns (port, connected_modbus) or (None, None).
    """
    for port in candidate_ports:
        mb = ModbusWrapper(port=port, slave_id=slave_id, baudrate=baudrate)
        if not mb.connect():
            mb.close()
            continue

        # Probe: try reading 1 register from address 0x0000
        probe = mb.read_block(0x0000, 1)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, modbus: ModbusWrapper, detected_port: Optional[str]):
        super().__init__()
        self.modbus = modbus
        ...
        if detected_port:
            self.status.showMessage(f"Connected on {detected_port}")
            self.timer.start()
        else:
            self.status.showMessage("Failed to detect RS485 port / Modbus slave not responding")
...
def main():
    ...
    # If detection failed, still create a ModbusWrapper (so GUI can open and show error)
    if modbus is None:
        modbus = ModbusWrapper(port=candidate_ports[0], slave_id=slave_id, baudrate=baudrate)

    app = QtWidgets.QApplication(sys.argv)

    win = MainWindow(modbus, detected_port)

    # Full screen 1024x600
    win.setGeometry(0, 0, 1024, 600)
    win.setFixedSize(1024, 600)
    win.showFullScreen()
